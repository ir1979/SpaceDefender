...
class PlayerProfile:
    ...
    def from_dict(data: dict) -> "PlayerProfile":
        ...
        # and use total_coins (not mid-game saved coins which are lost).
        profile.current_score = 0
        profile.current_coins = profile.total_coins  # Use total accumulated coins
    ...
    def add_weapon(self, weapon_name: str):
        ...
        logger.info(
            f"Profile '{self.name}': Added weapon '{weapon_name}'. Total: {self.weapon_inventory[weapon_name]}"
        )

    def use_weapon(self, weapon_name: str) -> bool:
        """Use a weapon from inventory. Returns True if successful, False if not available."""
        if (
            weapon_name in self.weapon_inventory
            and self.weapon_inventory[weapon_name] > 0
        ):
            self.weapon_inventory[weapon_name] -= 1
            logger.info(
                f"Profile '{self.name}': Used weapon '{weapon_name}'. Remaining: {self.weapon_inventory[weapon_name]}"
            )
            return True
    ...
    def start_new_game(self):
        ...
        self.current_coins = (
            self.total_coins
        )  # Start with the total coins from the profile
        self.current_level = 1
        self.session_start_coins = self.total_coins  # Remember what we started with
        self.games_played += 1
        logger.info(
            f"Profile '{self.name}': Starting new game. "
    ...
    def end_game(self, score: int, coins: int, level: int, time_played: float):
        logger.info(
            f"Profile '{self.name}': Ending game. Adding score: {score}, coins: {coins}."
        )
        logger.info(
            f"  - Before: total_score={self.total_score}, total_coins={self.total_coins}"
        )
        ...
        self.last_played = time.time()
        logger.info(
            f"  - After: total_score={self.total_score}, total_coins={self.total_coins}"
        )
    ...
    def apply_purchase(self, cost: int):
        ...
        """
        logger.debug(
            f"Profile '{self.name}': Applying purchase cost: {cost}. Current coins: {self.current_coins}"
        )
    ...
    def apply_upgrades(self, player):
        """Apply persistent shop upgrades to a player object."""
        player.max_health = player_config.HEALTH + self.upgrade_levels.get("max_health", 0) * 20
        player.health = min(getattr(player, "health", player.max_health), player.max_health)
        player.damage = player_config.DAMAGE + self.upgrade_levels.get("damage", 0) * 10
        player.speed = min(
            player_config.SPEED + self.upgrade_levels.get("speed", 0),
            12,
        )
        player.fire_rate = max(
            player_config.FIRE_RATE - self.upgrade_levels.get("fire_rate", 0) * 2,
            3,
        )
        ...
        if self.weapon_inventory:
            ...
            if player.selected_weapon_index >= len(player.weapons):
...
class SaveSystem:
    ...
    def get_profile_names() -> List[str]:
        ...
        filtered = [
            n for n in raw_names if "create new profile" not in n.strip().lower()
        ]
        return filtered
